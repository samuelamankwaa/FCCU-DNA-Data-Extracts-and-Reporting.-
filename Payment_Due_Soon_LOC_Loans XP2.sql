...
select distinct
	...
	--replicate('0',4-len(rtrim(Cphone3.number)))+rtrim(Cphone3.number),
	...
	, B.LTDESCP as type
	, l.next_due_date
	, phone = COALESCE(right(cast('000' as char(3))+rtrim(cast(Cphone1.area_code as char(3))),3)+
	...
	, HOME_PHONE = COALESCE(right(cast('000' as char(3))+rtrim(cast(HPhone.area_code as char(3))),3)+
...
From 
	ID.MonthEnd_DL_Load_Dates eom
  
	INNER JOIN LOAN  L
		on eom.dl_load_date =  L.DL_LOAD_DATE
		-- and  eom.sequence =1 -- comment out for testing
...
where  
	eom.dl_load_Date = @DL_Load_Date
	AND mp.participation_type = 101
	and ind.death_date is  null
	AND  L.branch_nbr <> 63
	and ((is_written_off is null) or (is_written_off = 0))
	and ((closed is null) or (closed = 0))
	and l.loan_type in (60,95)
	and not exists 
		(
			select * 
			from membershipwarning mw -- COLLECTION , BANKRUPTCY CODES
			where  
				mw.WARNING_CODE in  (4,5,6,8,9,10,18)
                and mem.member_nbr =mw.member_nbr
                and mem.dl_load_date=mw.dl_load_date
		)
	and ind.d1name not like'%TRUST%'
	and l.balance >0
	and DATEDIFF(d,@DL_Load_Date,l.next_due_date)=4
	and not exists 
		( 
			select * 
			from   AGR_LOANDELQ  agr_del
			where 
				l.member_nbr =agr_del.member_nbr
